...
from pre_dateset_Contrast import load,load_test_apply,get_wordDic_Embedding,batch_gen_with_test,batch_gen_with_list_wise,get_overlap_dict
import operator
...
def test_point_wise():
	...
	with tf.Graph().as_default():
		...
		with sess.as_default(),open(precision,"w") as log:
			...
			predicted_test = predict(sess,cnn,test_length,alphabet,FLAGS.batch_size,q_max_sent_length,d_max_sent_length)
			print("start evaluation test!!")
			map_NDCG0_NDCG1_ERR_p_test = evaluation_test.evaluationBypandas(test_length,predicted_test[:,-1])
			# sess.run(tf.global_variables_initializer())
			print ("query length {}:test epoch:map,NDCG0,NDCG1,ERR,p {}".format(length,map_NDCG0_NDCG1_ERR_p_test))
